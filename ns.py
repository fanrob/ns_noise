...
# === Обучение
def train(model, windows, optimizer=None, beta=0.5, epochs=20, batch_size=128, lr=5e-4, \
            autogenerate = False, fun_gen = None, device = torch.device('cpu')):
    # Если оптимизатор не передан — создаём новый
    if optimizer is None:
        optimizer = torch.optim.Adam(model.parameters(), lr)

    base_loss = nn.L1Loss() #вычислаем среднюю абсолютную ошибку (разницу между полученым и ожидаемым значением)
    #base_loss = nn.MSELoss()  #вычисляет среднеквадратичную ошибку между предсказанными и фактическими значениями
    #base_loss = nn.SmoothL1Loss(beta=beta)  #вычисляет или абсолютную ошибку, или квадратичную ошибку в зависимости от величины ошибки
    
    


############## генератор динамических данных ##############
    ...
    for epoch in range(epochs):
        ...
        if autogenerate:
            ...
            values = (values - values.mean()) / values.std()  # нормализация
            windows = create_windows(values, SEQ_LEN, PRED_LEN)
    ...
    #    base_loss = nn.SmoothL1Loss(beta=t_beta)  # Huber Loss
        ...
        for i in range(0, len(windows), batch_size):
            ...
            outputs = model(inputs)                      # out = [batch,seq_len, 1]
            
            loss = base_loss(outputs, targets)
            #weights = torch.linspace(1.0, 0.2, steps=targets.size(0)).to(device)  # [seq_len]   # массив влияния местоположения элемента на его важность при вычислении ошибки
            #loss = (torch.abs(outputs - targets).squeeze(-1) * weights.unsqueeze(1)).mean()
        ...
        curTime = time.time()
        curloss = sum(batch_losses)/len(batch_losses)
      
        print(f"Epoch {epoch}, Time: {curTime - startTime:.2f}s, Loss: {curloss:.4f}, Max Loss: {max(batch_losses):.4f} ")
        startTime = curTime
